# Phase B-2: Baseline-failure audit (30 fail → bucket A/B/C, fix only C)

Read docs/m3-baseline.txt (30-fail list). For each failing test, check whether it references a token name (var(--md-*)), a CSS class (spectral-active, btn-primary, etc.), a component path under src/design-system/ or src/features/, or any of the M3 §10-violation fixes we just landed (AuthScreen gradient strip / MeetingActiveHeader dot / Progress wavy).
...
Report counts: A / B / C.
Re-run pnpm test. New failure total should be ≤ 30. If B and C fixes worked, count goes DOWN.
...
Deliverable:
  ...
  - Bucket A/B/C count
  - Final pnpm test count vs docs/m3-baseline.txt 30 fail
